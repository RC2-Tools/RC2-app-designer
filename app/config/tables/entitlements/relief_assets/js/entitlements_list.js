      ...
      .then(function(result) {
        if (result.getCount() > 0) {
            odkTables.openDetailView(
              ...
              'config/tables/deliveries/relief_assets/html/deliveries_detail.html'
            )
        }
      })
      .catch(function(reason) {
            ...
            if (action === 'detail') {
                ...
                if (util.getWorkflowMode() === util.workflow.idOnly) {
                    ...
                    if (canDeliver === true) {
                        ...
                        reliefDataUtil.triggerAuthorizationDelivery(authId, beneficiaryEntityId,
                          actionAddCustomDelivery, jsonMap);
                    } else {
                        util.displayError(odkCommon.localizeText(locale, 'beneficiary_not_entitled'));
                    }
                } else {
                    reliefDataUtil.triggerEntitlementDelivery(entId, actionAddCustomDelivery);
                }
            }
        }
    }
